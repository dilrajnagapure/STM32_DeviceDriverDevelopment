		...
		if(pvI2C_handle->RxTxState == I2C_BUSY_TX)
		{
			...
			if(pvI2C_handle->pvI2Cx->I2C_SR1 & (1<<TxE))
			{
				...
				if(pvI2C_handle->length==0)
				{
					...
					//We may have to write the address of device and then memory address
					// and then read the data from the memory address as it is expected
					//in I2C FRAM
