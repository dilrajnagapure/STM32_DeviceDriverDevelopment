...
void i2c_write_read_interrupt(void)
{
	...
	//write 3 byte data to memory address 0x00 0x00
	I2C_MasterSendDataIT(&i2c1, Txbuffer, 5, 0x50);
	for(uint32_t i=0;i<1000;i++);

	//In order to read set repeated start as the seqence to read is
	//S ADDR W A MEMORY_ADDR W Sr ADDR R BYT1 BYT2 BYT3
	i2c1.Sr = SET;
	I2C_MasterSendDataIT(&i2c1, Txbuffer, 2, 0x50);
	for(uint32_t i=0;i<1000;i++);
	I2C_MasterReceiveDataIT(&i2c1, Rxbuffer, 3, 0x50);
	for(uint32_t i=0;i<1000;i++);
}
void led_blinking()
{
